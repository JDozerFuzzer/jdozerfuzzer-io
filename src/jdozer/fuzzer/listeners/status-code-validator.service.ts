...
@Injectable()
export class StatusCodeValidatorSub implements EventConsumer, OnModuleInit {

    private readonly logger = new Logger(StatusCodeValidatorSub.name);
    ...
    onModuleInit() {
        ...
        this.logger.log("StatusCodeValidatorSub registered");
    }
...
@Injectable()
export class StatusCodeValidator implements OnModuleInit {

    private readonly logger = new Logger(StatusCodeValidator.name);
    private readonly keyManager = new KeyManager();

    constructor(
        private readonly redisService: RedisService,
        private readonly redisPubSub: RedisPubSub,
        private readonly sub: StatusCodeValidatorSub
    ) { }
    ...
    public async validate(fuzzerId: UUID, responseId: UUID) {
        try {
            ...
            if (keys.length === 1) {
                ...
                await this.redisService.set(keys[0].replaceAll("RES", "statusCode"), validation);
                validation.operationid = op.name;
                await this.redisPubSub.publish("jdozer:fuzzer:listeners", fuzzerId, "validation", "status-code", validation);
    ...
    private findMatch(statusCode: number, op: any, respId: UUID): any {
        ...
        if (resp.length === 1) {
            return this.buildLevel(respId, "exact", statusCode, resp[0].statusCode);
        }

        const wildcard = statusCode.toString().charAt(0).concat('XX');
        resp = op.res.filter((r: any) => r.statusCode === wildcard);
        if (resp.length === 1) {
            return this.buildLevel(respId, "wildcard", statusCode, resp[0].statusCode);
        }

        resp = op.res.filter((r: any) => r.statusCode === 'default');
        if (resp.length === 1 && !this.is5xx(statusCode)) {
            return this.buildLevel(respId, "default", statusCode, resp[0].statusCode);
        }

        if (this.is5xx(statusCode)) {
            return this.buildLevel(respId, `5xx`, statusCode, undefined);
        }

        return this.buildLevel(respId, `none`, statusCode, undefined);
    }
    ...
    private buildLevel(responseId: UUID, type: `exact` | `wildcard` | `default` | `5xx` | `none`, statusCode: number, matched?: string): any {
        return {
            id: responseId,
            matchType: type,
